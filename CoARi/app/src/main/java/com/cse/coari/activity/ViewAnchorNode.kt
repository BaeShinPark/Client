...
class ViewAnchorNode: AnchorNode() {

    private val viewNode = Node().also { it.setParent(this) }
    ...
    private fun scaleNode(
        ...
        viewNode.localScale = when (videoScaleType) {
    ...
    private fun rotateNode(videoRotation: Float) {
        viewNode.localRotation = Quaternion.axisAngle(Vector3(0.0f, -1.0f, 0.0f), videoRotation)
    }
